...
function Begin($Request, $Post_ID){
	$_SESSION['Page Name'] = "Accessories Post $Post_ID";
	if ( $Request->isPOST() )
...
function Post_GET($Request, $Post_id){
	...
	$Result = $MySql->FetchOneRow('SELECT * FROM lux WHERE id = ? AND deleted = ?',
			array( $Post_id , '0'));

	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page("Accessories Post $Post_id");
	...
	return (new SiteRenderEngine())->Lux_Post_Render(
...
function Post_POST($Request, $Post_id){
	...
	*/
	$MySql = new ModelExcutionEngine();
	$Result = $MySql->FetchOneRow('SELECT * FROM lux WHERE id = ? AND deleted = ? AND status = ?',
			array( $Post_id , '0', '1'));
	...
	*/
	...
	Redirect(Post.'Lux/'.$Post_id.'/Message');
}
