...
function Begin($Request, $Post_ID){
	$_SESSION['Page Name'] = "Doc Post $Post_ID";
	if ( $Request->isPOST() )
...
function Post_GET($Request, $Post_id){
	...
	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page("Doctors Post $Post_id");
	...
	return (new SiteRenderEngine())->Doc_Post_Render(
...
function Post_POST($Request, $Post_id){
	...
	*/
	$MySql = new ModelExcutionEngine();
	$Result = $MySql->FetchOneRow('SELECT * FROM doc WHERE id = ? AND deleted = ? AND status = ?',
			array( $Post_id , '0', '1'));
	...
	*/
	...
	Redirect(Post.'Doc/'.$Post_id.'/Message');
}
