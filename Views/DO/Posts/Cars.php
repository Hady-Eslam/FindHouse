...
function Begin($Request, $Post_ID){
	$_SESSION['Page Name'] = "Car Post $Post_ID";
	if ( $Request->isPOST() )
...
function Post_GET($Request, $Post_id){
	...
	$Result = $MySql->FetchOneRow('SELECT * FROM cars WHERE id = ? AND deleted = ?',
			array( $Post_id , '0'));

	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page("Car Post $Post_id");
	...
	$User_Email = $Hashing->Get_Data_From_Hash([
		['Type' => 'CARS', 'Data' => $Data['user_email'], 'Key' => 'Email' ]
	]);
	...
	return (new SiteRenderEngine())->Cars_Post_Render(
...
function Post_POST($Request, $Post_id){
	...
	*/
	$MySql = new ModelExcutionEngine();
	$Result = $MySql->FetchOneRow('SELECT * FROM cars WHERE id = ? AND deleted = ? AND status = ?',
			array( $Post_id , '0', '1'));

	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page("Cars Post $Post_id");
	...
	*/
	$Hashing = new HashingEngine();
	$Result = $Hashing->Get_Hashed_CARS($Result->Data['user_email']);
	if ( $Result->Result != 1 )
		return (new SiteRenderEngine())->Error_Page("Cars Post $Post_id");
	...
	*/
	...
	Redirect(Post.'Cars/'.$Post_id.'/Message');
}
