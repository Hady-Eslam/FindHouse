...
function Begin($Request, $Post_ID){
	$_SESSION['Page Name'] = "Ant Post $Post_ID";
	if ( $Request->isPOST() )
...
function Post_GET($Request, $Post_id){
	...
	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page("Antiques Post $Post_id");
	...
	return (new SiteRenderEngine())->Ant_Post_Render(
...
function Post_POST($Request, $Post_id){
	...
	*/
	$MySql = new ModelExcutionEngine();
	$Result = $MySql->FetchOneRow('SELECT * FROM ant WHERE id = ? AND deleted = ? AND status = ?',
			array( $Post_id , '0', '1'));
	...
	*/
	...
	Redirect(Post.'Ant/'.$Post_id.'/Message');
}
