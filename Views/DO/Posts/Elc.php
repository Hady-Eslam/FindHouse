...
function Begin($Request, $Post_ID){
	$_SESSION['Page Name'] = "Elc Post $Post_ID";
	if ( $Request->isPOST() )
...
function Post_GET($Request, $Post_id){
	...
	$Result = $MySql->FetchOneRow('SELECT * FROM elc WHERE id = ? AND deleted = ?',
			array( $Post_id , '0'));

	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page("Electrical Post $Post_id");
	...
	return (new SiteRenderEngine())->Elc_Post_Render(
...
function Post_POST($Request, $Post_id){
	...
	*/
	$MySql = new ModelExcutionEngine();
	$Result = $MySql->FetchOneRow('SELECT * FROM elc WHERE id = ? AND deleted = ? AND status = ?',
			array( $Post_id , '0', '1'));
	...
	*/
	...
	Redirect(Post.'Elc/'.$Post_id.'/Message');
}
