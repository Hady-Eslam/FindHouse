...
function Begin($Request, $Post_ID){
	$_SESSION['Page Name'] = "Fashion Post $Post_ID";
	if ( $Request->isPOST() )
...
function Post_GET($Request, $Post_id){
	...
	$Result = $MySql->FetchOneRow('SELECT * FROM fashion WHERE id = ? AND deleted = ?',
			array( $Post_id , '0'));

	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page("Fashion Post $Post_id");
	...
	$User_Email = $Hashing->Get_Data_From_Hash([
		['Type' => 'ELC', 'Data' => $Data['user_email'], 'Key' => 'Email' ]
	]);
	...
	return (new SiteRenderEngine())->Fashion_Post_Render(
...
function Post_POST($Request, $Post_id){
	...
	*/
	$MySql = new ModelExcutionEngine();
	$Result = $MySql->FetchOneRow('SELECT * FROM fashion WHERE id = ? AND deleted = ? AND status = ?',
			array( $Post_id , '0', '1'));
	...
	*/
	$Hashing = new HashingEngine();
	$Result = $Hashing->Get_Hashed_ELC($Result->Data['user_email']);
	if ( $Result->Result != 1 )
		return (new SiteRenderEngine())->Error_Page("Fashion Post $Post_id");
	...
	*/
	...
	Redirect(Post.'Fashion/'.$Post_id.'/Message');
}
