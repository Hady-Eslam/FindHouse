...
function Begin($Request, $Post_ID){
	$_SESSION['Page Name'] = "Eat Post $Post_ID";
	if ( $Request->isPOST() )
...
function Post_GET($Request, $Post_id){
	...
	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page("Eat Post $Post_id");
	...
	$User_Email = $Hashing->Get_Data_From_Hash([
		['Type' => 'EAT', 'Data' => $Data['user_email'], 'Key' => 'Email' ]
	]);
	...
	return (new SiteRenderEngine())->Eat_Post_Render(
...
function Post_POST($Request, $Post_id){
	...
	*/
	$MySql = new ModelExcutionEngine();
	$Result = $MySql->FetchOneRow('SELECT * FROM eat WHERE id = ? AND deleted = ?  AND status = ?',
			array( $Post_id , '0', '1'));
	...
	*/
	$Hashing = new HashingEngine();
	$Result = $Hashing->Get_Hashed_EAT($Result->Data['user_email']);
	if ( $Result->Result != 1 )
		return (new SiteRenderEngine())->Error_Page("Eat Post $Post_id");
	...
	*/
	...
	Redirect(Post.'Eat/'.$Post_id.'/Message');
}
