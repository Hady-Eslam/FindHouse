...
use Forms\BackEndForms\FindForms\SearchAntForm;
...
function Begin($Request){
	...
	return (new SiteRenderEngine())->Not_Authurithed_User('Find Antiques Tools');
}

function GetMorePosts($POST){

	$Form = new SearchAntForm($POST);
	if ( !$Form->isValid() )
		return (new SiteRenderEngine())->Not_Authurithed_User('Find Antiques Tools');

	
	$Result = (new ModelExcutionEngine())->FetchAllRows(
		'SELECT * FROM ant WHERE deleted = ? AND status = ? LIMIT '
			.$Form->GetLimit().' , 21',
		array(
			'0',
			'1'
	...
	if ( $Result->Result == -1 )
		return (new SiteRenderEngine())->Error_Page('Find Antiques Tools');
